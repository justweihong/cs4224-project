...
async function newOrderTransaction(callbackHadler, client, W_ID, D_ID, C_ID, NUM_ITEMS, ITEM_NUMBER, SUPPLIER_WAREHOUSE, QUANTITY) {
    ...
    var D_TAX = 0;
    await client.execute('SELECT D_TAX FROM Districts WHERE D_ID = ' + D_ID + ' AND D_W_ID = ' + W_ID).then(res => {
        D_TAX = res.rows[0].d_tax;
    }).catch(err => {
        console.error(err.stack);
    });
    var C_DISCOUNT = 0;
    await client.execute('SELECT C_DISCOUNT FROM Customers WHERE C_ID = ' + C_ID + ' AND C_D_ID = ' + D_ID + ' AND C_W_ID = ' + W_ID).then(res => {
    ...
    var C_LAST = 0;
    await client.execute('SELECT C_LAST FROM Customers WHERE C_ID = ' + C_ID + ' AND C_D_ID = ' + D_ID + ' AND C_W_ID = ' + W_ID).then(res => {
        C_LAST = res.rows[0].c_last;
    }).catch(err => {
        console.error(err.stack);
    });
    var C_CREDIT = 0;
    await client.execute('SELECT C_CREDIT FROM Customers WHERE C_ID = ' + C_ID + ' AND C_D_ID = ' + D_ID + ' AND C_W_ID = ' + W_ID).then(res => {
